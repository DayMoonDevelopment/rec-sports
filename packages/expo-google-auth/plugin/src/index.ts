...
const withGoogleAuth: ConfigPlugin<GoogleAuthPluginProps> = (config, props) => {
  if (!props?.iosClientId) {
    console.error("GoogleAuth plugin requires iosClientId");
...
const withGoogleAuthIOS: ConfigPlugin<GoogleAuthPluginProps> = (
  config,
  ...
  return withInfoPlist(config, (config) => {
    ...
    const existingGoogleScheme = (
      config.modResults.CFBundleURLTypes as any[]
    ).find((urlType: any) =>
...
const withGoogleAuthAndroid: ConfigPlugin<GoogleAuthPluginProps> = (
  config,
  { webClientId },
) => {
...
const pkg = {
  name: "@rec/expo-google-auth-plugin",
  version: "1.0.0",
};
